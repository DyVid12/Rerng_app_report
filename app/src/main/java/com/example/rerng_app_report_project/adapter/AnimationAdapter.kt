...
class AnimationAdapter : ListAdapter<Movy, AnimationAdapter.AnimationViewHolder>(AnimationDiffUtil) {

    class AnimationViewHolder(private val binding: ViewHolderDataBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bindAnimationDataMovie(movy: Movy) {
            if (!movy.poster.isNullOrEmpty()) {
                ...
                binding.image1.setOnClickListener { v ->
                    ...
                    val intent = Intent(context, MovieDetailActivity::class.java).apply {
                        ...
                        Log.d("AnimationAdapter", "Passing Rating: $ratingValue")
    ...
    override fun onBindViewHolder(holder: AnimationViewHolder, position: Int) {
        holder.bindAnimationDataMovie(getItem(position))
    }
}

object AnimationDiffUtil : DiffUtil.ItemCallback<Movy>() {
