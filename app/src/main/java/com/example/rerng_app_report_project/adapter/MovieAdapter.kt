...
// MovieAdapter
class MovieAdapter : ListAdapter<Movy, MovieAdapter.DataMovieViewHolder>(DataMovieDiffCallback()) {

    // ViewHolder
    class DataMovieViewHolder(private val binding:ViewHolderDataBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bindDataMovies(movy: Movy) {
            if (!movy.poster.isNullOrEmpty()) {
                val baseUrl = "http://10.0.2.2:8000/" // Replace with the actual base URL
                ...
                binding.image1.setOnClickListener { v ->
                    ...
                    val intent = Intent(context, MovieDetailActivity::class.java).apply {
                        ...
                        Log.d("MovieAdapter", "Passing Rating: $ratingValue")
    ...
    // Create ViewHolder
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DataMovieViewHolder {
        val binding = ViewHolderDataBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return DataMovieViewHolder(binding)
    }

    // Bind Data
    override fun onBindViewHolder(holder: DataMovieViewHolder, position: Int) {
        val movie = getItem(position)
        holder.bindDataMovies(movie)
    }
}

class DataMovieDiffCallback : DiffUtil.ItemCallback<Movy>() {
    override fun areItemsTheSame(oldItem: Movy, newItem: Movy): Boolean {
        return oldItem.id == newItem.id // Check by ID
    }

    override fun areContentsTheSame(oldItem: Movy, newItem: Movy): Boolean {
        return oldItem == newItem// Check by content equality
    }
